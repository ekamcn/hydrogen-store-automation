...
export default function StepIndicator({
  ...
  return (
    <div className="w-full min-h-[100px] pb-2">
      {/* Desktop Version */}
          ...
          <ol role="list" className="flex items-center">
            {steps.map((step, index) => {
              ...
              return (
                <li
                  key={step.id}
                  className={`relative ${index !== steps.length - 1 ? 'pr-8 sm:pr-20 flex-grow' : ''}`}
                >
                  {/* Connection line */}
                  <div className="absolute inset-0 flex items-center pt-3" aria-hidden="true">
                    <div
                      className={`h-0.5 w-full ${
                        index <= currentStep ? 'bg-[#40E0D0]' : 'bg-[#E9967A]'
                      }`}
                    />
                  </div>
                  ...
                  <button
                    ...
                    className={`relative flex h-8 w-8 items-center justify-center rounded-full transition-all duration-200 ${
                      isCurrent
                        ...
                        : isCompleted && isValid
                        ? 'bg-[#0000ff] hover:bg-[#0000ff]'
                        : hasErrors
                        ? 'bg-[#ff0000] hover:bg-[#ff0000] border-2 border-red-500'
                  ...
                  <div className="mt-2 h-6">
                    <span
                      className={`text-sm font-medium flex items-center gap-1 w-[70px] ${
                        hasErrors ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-gray-100'
                      }`}
                    >
                      {step.name}
                      {hasErrors && (
                        <span className="text-xs text-[#ff0000]" title="Please complete required fields">
                          *
                        </span>
                      )}
                    </span>
