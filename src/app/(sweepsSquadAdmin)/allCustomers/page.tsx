            ...
            <Table className="min-w-[650px] sm:min-w-[750px] md:min-w-[900px]">
              ...
              <TableHeader>
                <TableRow className="bg-gray-50">
                  <TableHead className="px-3 sm:px-4 py-2 sm:py-3 text-gray-700 text-xs sm:text-sm font-semibold">S.No.</TableHead>
                  <TableHead className="px-3 sm:px-4 py-2 sm:py-3 text-gray-700 text-xs sm:text-sm font-semibold">Store Name</TableHead>
                  <TableHead className="px-3 sm:px-4 py-2 sm:py-3 text-gray-700 text-xs sm:text-sm font-semibold">Theme Status</TableHead>
                  <TableHead className="px-3 sm:px-4 py-2 sm:py-3 text-gray-700 text-xs sm:text-sm font-semibold">Store URL</TableHead>
                  <TableHead className="px-3 sm:px-4 py-2 sm:py-3 text-gray-700 text-xs sm:text-sm font-semibold">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {stores.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="h-24 text-center text-gray-400">No stores found.</TableCell>
                  </TableRow>
                ) : (
                  stores.map((customer, idx) => (
                    ...
                    >
                      <TableCell className="px-3 sm:px-4 py-2 sm:py-3 text-gray-700 font-medium text-center text-sm">{idx + 1}</TableCell>
                      <TableCell className="px-3 sm:px-4 py-2 sm:py-3 text-gray-800 text-sm">{customer.storeName}</TableCell>
                      ...
                      <TableCell className="px-3 sm:px-4 py-2 sm:py-3">
                        <div className="flex flex-col sm:flex-row gap-2">
                          <Button
                            onClick={() => window.open(customer?.storeUrl, '_blank')}
                            disabled={loading}
                            variant="outline"
                          ...
                          <Button
                            onClick={async () => {
                              try {
                                ...
                                router.push(`/storeEditor?storeName=${encodeURIComponent(customer.storeName)}`);
                              }
                            }}
                            disabled={loading}
                            variant="default"
              ...
              <TableFooter>
                <TableRow>
                  <TableCell colSpan={5} className="text-left text-xs sm:text-sm text-muted-foreground px-3 sm:px-4 py-2 sm:py-3">
