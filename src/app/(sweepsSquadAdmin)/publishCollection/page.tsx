...
export default function CsvPublishPage() {
  ...
  const [failedCollectionRecords, setFailedCollectionRecords] = useState<
    FailedCollectionRecord[]
  >([]);
  // const [failedProductRecords, setFailedProductRecords] = useState<FailedProductRecord[]>([]);
  const [successfulCollectionRecords, setSuccessfulCollectionRecords] =
    useState<SuccessfulCollectionRecord[]>([]);
  ...
  useEffect(() => {
    ...
    socket.on(
      ...
      (
        ...
        console.log(
          "DEBUG: publish:progress payload:",
    ...
    socket.on(
      ...
      (payload: { scope: string; successCount?: number }) => {
        ...
        console.log(
          "DEBUG: publish:completed payload:",
    ...
    socket.on(
      ...
      (
        ...
        console.log(
          "DEBUG: publish:collections:progress payload:",
          JSON.stringify(progress)
        );
        setCollectionProcessingStatus((prev) => ({ ...prev, ...progress }));
        // const statusMessage =
    ...
    socket.on(
      ...
      (payload: {
        ...
        console.log(
          "DEBUG: publish:collections:error payload:",
          JSON.stringify(payload)
        );
        setCollectionError(payload?.message || "Collection error");
    ...
    socket.on(
      ...
      (payload: { created: number; total: number }) => {
        ...
        console.log(
          "DEBUG: publish:collections:done payload:",
          JSON.stringify(payload)
        );
        setCollectionSuccess(
          `Collections done: ${payload.created}/${payload.total}`
        );
      }
    );
    ...
    socket.on("publish:failedRecords", (failed: FailedCollectionRecord[]) => {
      ...
      console.log(
        "DEBUG: publish:failedRecords payload:",
    ...
    socket.on(
      ...
      (payload: {
        ...
        console.log(
          "DEBUG: publish:collections:published payload:",
          JSON.stringify(payload)
        );
        setSuccessfulCollectionRecords((prev) => {
          if (
            prev.some(
              (rec) =>
                rec.title === payload.title && rec.handle === payload.handle
            )
          ) {
          ...
          return [
            ...prev,
            { title: payload.title, handle: payload.handle, ...payload },
          ];
        });
      }
    );
    ...
    socket.on(
      "publish:collections:publish_error",
      (payload: {
        message?: string;
        title?: string;
        handle?: string;
        ...
        console.log(
          "DEBUG: publish:collections:publish_error payload:",
          JSON.stringify(payload)
        );
    ...
    socket.on("publish:collections:publish_summary", (payload: unknown) => {
      ...
      console.log(
        "DEBUG: publish:collections:publish_summary payload:",
        JSON.stringify(payload)
      );
    });
    ...
    socket.on(
      ...
      (payload: {
        ...
        console.log(
          "DEBUG: publish:collections:success payload:",
          JSON.stringify(payload)
        );
      }
    );
    ...
    socket.on("publish:category_language", (payload: unknown) => {
      ...
      console.log(
        "DEBUG: publish:category_language payload:",
        JSON.stringify(payload)
      );
    });
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      {generalSuccess && (
        <SuccessMessage message={generalSuccess} className="mb-4" />
      )}

      {/* Collections Section */}
      <h2 className="text-2xl font-semibold mb-4">Collections</h2>
      {collectionStatus && (
        <div className="mb-4 text-blue-700">{collectionStatus}</div>
      )}
      {collectionError && (
        <ErrorMessage message={collectionError} className="mb-4" />
      )}
      {collectionSuccess && (
        <SuccessMessage message={collectionSuccess} className="mb-4" />
      )}
          ...
          <div className="grid grid-cols-3 gap-4 mb-3">
            <div>
              ...
              <p className="text-xl font-medium">
                {collectionProcessingStatus.total}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Successful</p>
              <p className="text-xl font-medium">
                {collectionProcessingStatus.successful}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Failed</p>
              <p className="text-xl font-medium">
                {collectionProcessingStatus.failed}
              </p>
            </div>
          </div>
          <div className="mt-3 w-full bg-gray-200 rounded-full h-2.5">
            <div
              ...
              style={{
                width: `${
                  collectionProcessingStatus.total > 0
                    ? (collectionProcessingStatus.processed /
                        collectionProcessingStatus.total) *
                      100
                    : 0
                }%`,
          ...
          {(failedCollectionRecords.length > 0 ||
            ...
            <div className="mt-4">
              {failedCollectionRecords.length > 0 && (
                <Button
                  onClick={() =>
                    exportToCSV(failedCollectionRecords, "failed-collections")
                  }
                  variant="outline"
          ...
          <div className="overflow-x-auto max-h-60">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-green-100">
                <tr>
                  <th className="px-3 py-2 text-left text-xs font-medium text-green-700">
                    Title
                  </th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-green-700">
                    Handle
        ...
        <Card className="p-6 mb-6 border-red-200 bg-red-50">
          ...
          <div className="overflow-x-auto max-h-60">
            ...
            {failedCollectionRecords.length > 5 && (
              <p className="mt-2 text-xs text-gray-500 italic">
                Showing first 5 of {failedCollectionRecords.length} failed
                records. Download the CSV for complete details.
              </p>
            )}
          </div>
        </Card>
